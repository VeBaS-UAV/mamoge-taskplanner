...
def DAG_all_parallel(G_routemap, base_id, nodes):
    ...
    digraph_tasks.add_node(
        task_base_id,
        name="start",
        layer=0,
        location=NXLayerLocation(
            layer_id=task_base_id,
            base_id=base_id,
    ...
    [
        digraph_tasks.add_edge(task_id, end_id)
        for task_id in list(digraph_tasks.nodes)[1:-1]
    ]
