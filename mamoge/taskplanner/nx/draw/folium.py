...
def draw_folium_new_map(G: nx.Graph):
    origin = (
        np.array(list(nx.get_node_attributes(G, "y").values())).mean(),
        np.array(list(nx.get_node_attributes(G, "x").values())).mean(),
    )
    folium_map = folium.Map(
        location=origin, zoom_start=14, max_native_zoom=19, max_zoom=29
    )
...
def draw_folium_poi(G, poi_ids, folium_map=None, name="POI", show=True, **draw_args):
    ...
    for i, _id in enumerate(poi_ids):
        ...
        try:
            folium.Marker(
                location=(loc.y, loc.x), popup=tagname, icon=folium.Icon(**draw_args)
            ).add_to(fg_poi)
...
def draw_folium_path(
    G, path, folium_map=None, name="name", color=None, show=True, **draw_args
):
