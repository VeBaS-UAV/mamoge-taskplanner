...
class ORTaskOptimizer:
    ...
    def solve(self, max_time=30, num_routes=1, constraints=[]):
        ...
        for dim_name, dim_args in self.dimensions.items():
            ...
            capacity = (
                dim_args["capacity"] if dim_args["capacity"] is not None else 300000000
            )
            fix_start_cumul_to_zero = True

            # cost_matrix = mamogenx.G_cost_matrix(self.graph, lambda u,v: dim_cost_callback(self.graph, u,v))

            dim_matrix = np.zeros((len(self.graph), len(self.graph)), dtype=int)
            ...
            def cost_callback(from_index, to_index):
                ...
                try:
                    ...
                    self.logger.error(
                        (
                            f"cost_callback error ({from_index}, "
                            f"{to_index}), "
                            f"{cost_callback}"
                        )
                    )
            ...
            self.logger.info(
                (
                    f"Adding dimension {dim_name}: {slack}, "
                    f"{capacity}, "
                    f"{fix_start_cumul_to_zero}"
                )
            )
            self.routing.AddDimension(
                callback_index, slack, capacity, fix_start_cumul_to_zero, dim_name
            )
        ...
        for cap_name, cap_args in self.capacities.items():
            ...
            capacity = (
                cap_args["capacity"] if cap_args["capacity"] is not None else 100000000
            )
            fix_start_cumul_to_zero = True
        ...
        for constraint in constraints:
            ...
            self.routing.solver().Add(
                self.routing.VehicleVar(first_index)
                == self.routing.VehicleVar(second_index)
            )
            ...
            if "min" in kw_args:
                ...
                constrain_arg = (
                    dimension.CumulVar(first_index) + min_value
                ) <= dimension.CumulVar(second_index)
            ...
            if "max" in kw_args:
                ...
                dim_str = constraint.dimension
                constrain_arg = (
                    dimension.CumulVar(first_index) + max_value
                ) > dimension.CumulVar(second_index)
                self.routing.solver().Add(constrain_arg)
