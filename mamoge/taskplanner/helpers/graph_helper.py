...
def example_graph(extended_version=False):
    G = nx.DiGraph()
    G.add_node(0, layer=0, location=(0,2))
    #G.add_nodes_from([1,2,3,4], layer=1)
    [G.add_node(i, layer=1, location=(2,p)) for i,p in zip([1,2,3,4],(3,1,2,4))]
    G.add_node(5, layer=2, location=(4,3))
    G.add_node(6, layer=2, location=(6,2))

    if(extended_version):

        [G.add_node(i, layer=3, location=(8,p)) for i,p in zip([7,8,9],(3,1,2))]
    ...
    [G.add_edge(0,i, distance=mamogenx.G_distance_manhatten(G,0,i)) for i in range(1,5)]
    [G.add_edge(i,6, distance=mamogenx.G_distance_manhatten(G,i,6)) for i in range(2,5)]
    G.add_edge(1,5, distance=mamogenx.G_distance_manhatten(G,1,5))
    G.add_edge(5,6, distance=mamogenx.G_distance_manhatten(G,5,6))

    if (extended_version):
        [G.add_edge(6, i, distance=mamogenx.G_distance_manhatten(G,6, i)) for i in range(7,10)]
